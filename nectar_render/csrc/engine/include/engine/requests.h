...
public:
    ...
    void rebuild_materials() { rebuild_bvh_.store(true, relaxed); }
    void restart(
        const bool build_bvh = false,
        const bool build_materials = false
    ) { 
        if (build_bvh)       rebuild_bvh_.store(true, relaxed);
        if (build_materials) rebuild_materials_.store(true, relaxed);
    ...
    bool material_build_pending() { 
        return rebuild_materials_.exchange(false, relaxed); 
    }
...
private:
    ...
    std::atomic<bool> rebuild_materials_ { false };
